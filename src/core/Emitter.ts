...
export class Emitter extends AbstractEmitter {
    private _events: Map<string, IListener[]> = new Map();

    public getListeners(event: string): IListener[] {
        return this._events.get(event) || [];
    }

    public on(name: string, callback: ICallback, context?: unknown): this {
        this._addListener(name, callback, context, false);
        return this;
    }

    public once(name: string, callback: ICallback, context?: unknown): this {
        this._addListener(name, callback, context, true);
        return this;
    }

    public off(name: string, callback: ICallback, context?: unknown): this {
    ...
    public removeListenersOf(context: unknown): void {
        const keys = Array.from(this._events.keys());
    ...
    private _addListener(name: string, callback: ICallback, context: unknown, once: boolean): void {
        const ll: IListener[] = this.getListeners(name);
        const l = { callback, context, once };
        !ll.length ? this._events.set(name, [l]) : ll.push(l);
    }

    private _removeListener(name: string, callback: ICallback, context?: unknown): void {
        const ll: IListener[] = this.getListeners(name);
        if (!ll.length) {
            return;
        }
